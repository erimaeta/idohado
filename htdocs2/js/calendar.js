...
function generate_month_calendar(year, month) {
    ...
    cTable.className = 'EigyoCalendar__tbl';
    ...
    for (var i = 0; i < weekdayData.length; i++) {
        insertData += '<th class="EigyoCalendar__th">';
    ...
    for (var i = 0; i < calendarData.length; i++) {
        if(calendarData[i]['weekday'] <= 0) {
            insertData += '<tr class="EigyoCalendar__tr">';
        }
        ...
        if(calendarData[i]['day'] === ''){
            ...
            var eigyoFlag = false;
            if($eigyoArr.includes(calendarData[i]['weekday'])){
                var mynum = Number(calendarData[i]['day']);
                if($oyasumiArr.includes(mynum)===false){
                    eigyoFlag = true;
                }
            }
            if(eigyoFlag===true){
                insertData += '<td class="EigyoCalendar__td is-eigyo">';
            }else{
                insertData += '<td class="EigyoCalendar__td is-oyasumi">';
            }
        }
